    ...
    <div className={`target-card p-4 group animate-scale-in`}>
      ...
      <div className="flex items-baseline gap-1">
        <div className={`text-[48px] font-bold ${
          color === 'teal' ? 'text-accent-teal text-neon' : 'text-accent-pink text-neon'
      ...
      <div className="text-[10px] text-gray-300 uppercase mt-2 leading-tight group-hover:text-white transition-colors duration-300">
        {label}
        <div className="text-[9px] text-gray-400 mt-1 group-hover:text-gray-300 transition-colors duration-300">{sublabel}</div>
      </div>
    </div>
  );
}
...
export default function TacticalTracking() {
  ...
  const sortedData = [...tacticalData].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  const getCandidateData = (candidate: string) => {
    return uniqueDates.map(date => {
      const dataPoint = sortedData.find(item => item.date === date && item.candidate === candidate);
  ...
  const conocimientoVariance = getVariance(currentConocimiento, previousConocimiento);
  ...
  return (
    <div className="glassmorphic-container p-5 h-[480px] overflow-hidden animate-scale-in">
      ...
      <div className="flex justify-between items-center mt-2 text-xs text-gray-300">
        <div>
          ÚLTIMA ACTUALIZACIÓN: {new Date(uniqueDates[uniqueDates.length - 1]).toLocaleString('es-ES', {
            year: 'numeric',
            month: 'long',
            day: '2-digit',
            hour: '2-digit',
